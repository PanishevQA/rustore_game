...
Remove-Item $resultDump -Force -ErrorAction SilentlyContinue
$resultVerified = $false
foreach ($candidate in $resultCandidates) {
    $dump = & $javapExe -classpath $candidate.Jar -public $candidate.Class
    if ($LASTEXITCODE -ne 0) { continue }

    Add-Content $resultDump ("=== " + $candidate.Source + " :: " + $candidate.Class + " ===")
    $dump | Add-Content $resultDump

    if (($dump -join [Environment]::NewLine) -match 'getInstallReferrer\s*\(') {
        $dump | Out-Host
        $resultVerified = $true
        break
    }
...
if (-not $resultVerified) {
    if (Test-Path $resultDump) { Get-Content $resultDump }
