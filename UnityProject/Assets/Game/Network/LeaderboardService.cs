...
namespace DontGetSidetracked.Network
{
    ...
    public sealed class LeaderboardService
    {
        ...
        public async Task<LeaderboardSnapshot> LoadAsync(string challengeId, int limit = 50)
        {
            if (string.IsNullOrWhiteSpace(challengeId))
                throw new ArgumentException("Challenge id is required.", nameof(challengeId));

            LeaderboardDto dto = await _api.GetDailyLeaderboardAsync(challengeId, Math.Max(1, Math.Min(100, limit)));
