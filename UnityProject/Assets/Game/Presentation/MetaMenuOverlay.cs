...
namespace DontGetSidetracked.Presentation
{
    ...
    public sealed class MetaMenuOverlay : MonoBehaviour
    {
        ...
        private void RenderStatisticsRows()
        {
            ...
            string average = _save.TotalScoredAttempts > 0
                ? (_save.TotalScoreSum / _save.TotalScoredAttempts).ToString("0.0") + "%" : "—";
            AddMetricPair("СРЕДНИЙ РЕЗУЛЬТАТ", average, "ПОПЫТОК", _save.TotalScoredAttempts.ToString());
            ...
            AddStoreSectionLabel(_save.TotalScoredAttempts == 0
                ? "Средний результат появится после новой попытки."
                : "Средний результат — по сохранённым попыткам.", ReleaseUiComponents.Muted);
